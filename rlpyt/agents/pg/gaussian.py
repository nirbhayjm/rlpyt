...
from rlpyt.utils.buffer import buffer_func, buffer_method, buffer_to
...
class RecurrentGaussianPgAgentBase(BaseAgent):
    ...
    def step(self, observation, prev_action, prev_reward):
        ...
        agent_info = AgentInfoRnn(
            dist_info=dist_info, value=value, prev_rnn_state=prev_rnn_state
        )
        action, agent_info = buffer_to((action, agent_info), device="cpu")
    ...
    def value(self, observation, prev_action, prev_reward):
        ...
        _mu, _log_std, value, _rnn_state = self.model(
...
class AlternatingRecurrentGaussianPgAgent(
    AlternatingRecurrentAgentMixin, RecurrentGaussianPgAgentBase
):
    pass
