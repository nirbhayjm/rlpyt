...
class FrameBufferMixin:
    ...
    def __init__(self, example, **kwargs):
        ...
        buffer_example = BufferSamples(
            *(v for k, v in example.items() if k != "observation")
        )
        super().__init__(example=buffer_example, **kwargs)
        # Equivalent to image.shape[0] if observation is image array (C,H,W):
        self.n_frames = n_frames = get_leading_dims(example.observation, n_dim=1)[0]
    ...
    def append_samples(self, samples):
        ...
        buffer_samples = BufferSamples(
            *(v for k, v in samples.items() if k != "observation")
        )
