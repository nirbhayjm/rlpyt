...
class CppoPID(PolicyGradientAlgo):
    ...
    def __init__(
        ...
        penalty_init=1.0,
        cost_limit=25,
        cost_scale=1.0,  # divides; applied to raw cost and cost_limit
        ...
        cost_gae_lambda = gae_lambda if cost_gae_lambda is None else cost_gae_lambda
        cost_value_loss_coeff = (
            value_loss_coeff if cost_value_loss_coeff is None else cost_value_loss_coeff
        )
        ...
        self._beta_kl = 1.0
        self._beta_grad = 1.0
        self.beta_min = 1.0 / self.beta_max
    ...
    def initialize(self, *args, **kwargs):
        ...
        if self.step_cost_limit_steps is None:
            ...
            self.step_cost_limit_itr = int(
                self.step_cost_limit_steps // (self.batch_spec.size * self.world_size)
            )
    ...
    def optimize_agent(self, itr, samples):
        ...
        (
            ...
            c_advantage,
            ep_cost_avg,
        ) = self.process_returns(itr, samples)
        ...
        if self.step_cost_limit_itr is not None and self.step_cost_limit_itr == itr:
    ...
    def loss(
        ...
        if self.objective_penalized:
            ...
            if self.lagrange_quadratic_penalty:
                quad_loss = (
                    self.quadratic_penalty_coeff
                    * valid_mean(c_surrogate, valid)
                    * torch.max(torch.tensor(0.0), self._ep_cost_ema - self.cost_limit)
                )
                pi_loss += quad_loss
    ...
    def process_returns(self, itr, samples):
        ...
        if not self.mid_batch_reset or self.agent.recurrent:
            ...
            ep_cost_mask = valid * (
                1 - torch.cat([valid[1:], torch.ones_like(valid[-1:])])
        ...
        return (return_, advantage, valid, c_return, c_advantage, self._ep_cost_ema)

    def compute_beta_kl(self, loss_inputs, init_rnn_state, batch_size, mb_size, T):
        ...
        for _ in range(self.beta_kl_epochs):
            for idxs in iterate_mb_idxs(
                batch_size, mb_size, shuffle=batch_size > mb_size
            ):
                T_idxs = slice(None) if recurrent else idxs % T
                B_idxs = idxs if recurrent else idxs // T
                rnn_state = init_rnn_state[B_idxs] if recurrent else None
    ...
    def beta_kl_losses(
        ...
        if init_rnn_state is not None:
            ...
            r_dist_info, c_dist_info = self.agent.beta_dist_infos(*agent_inputs)
        dist = self.agent.distribution

        r_ratio = dist.likelihood_ratio(
            action, old_dist_info=old_dist_info, new_dist_info=r_dist_info
        )
        surr_1 = r_ratio * advantage
        r_clipped_ratio = torch.clamp(
            r_ratio, 1.0 - self.ratio_clip, 1.0 + self.ratio_clip
        )
        surr_2 = r_clipped_ratio * advantage
        surrogate = torch.min(surr_1, surr_2)
        beta_r_loss = -valid_mean(surrogate, valid)

        c_ratio = dist.likelihood_ratio(
            action, old_dist_info=old_dist_info, new_dist_info=c_dist_info
        )
        c_surr_1 = c_ratio * c_advantage
        c_clipped_ratio = torch.clamp(
            c_ratio, 1.0 - self.ratio_clip, 1.0 + self.ratio_clip
        )
