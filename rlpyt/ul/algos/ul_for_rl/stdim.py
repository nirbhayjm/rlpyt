...
from rlpyt.ul.models.ul.stdim_models import (
    StDimEncoderModel,
...
class STDIM(BaseUlAlgorithm):
    ...
    def initialize(self, n_updates, cuda_idx=None):
        ...
        local_size = self.encoder.conv_out_shapes[self.local_conv_layer][
            0
        ]  # C from [C,H,W]
    ...
    def stdim_loss(self, samples):
        ...
        labels = torch.arange(
            c_anchor.shape[0], dtype=torch.long, device=self.device  # batch size
        )
        valid = valid_from_done(samples.done).type(torch.bool)
        valid = valid[self.delta_T :].reshape(-1)
        labels[~valid] = IGNORE_INDEX

        gg_loss, gl_loss, ll_loss = [torch.tensor(0.0, device=self.device)] * 3
        ...
        if self.use_global_local:
            ...
            gl_losses = torch.stack(
                [self.c_e_loss(gl_logits, labels) for gl_logits in gl_logits_list]
            )
            gl_loss = torch.mean(gl_losses)
            gl_corrects = [
                torch.argmax(gl_logits.detach(), dim=1) == labels
                for gl_logits in gl_logits_list
            ]
            gl_accuracies = [
                torch.mean(gl_correct[valid].float()) for gl_correct in gl_corrects
            ]
        ...
        if self.use_local_local:
            ...
            ll_losses = torch.stack(
                [self.c_e_loss(ll_logits, labels) for ll_logits in ll_logits_list]
            )
            ll_loss = torch.mean(ll_losses)
            ll_corrects = [
                torch.argmax(ll_logits.detach(), dim=1) == labels
                for ll_logits in ll_logits_list
            ]
            ll_accuracies = [
                torch.mean(ll_correct[valid].float()) for ll_correct in ll_corrects
            ]
