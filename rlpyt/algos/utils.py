...
def discount_return_tl(
    reward, done, bootstrap_value, discount, timeout, value, return_dest=None
):
...
def generalized_advantage_estimation_tl(
    reward,
    value,
    done,
    bootstrap_value,
    ...
    for t in reversed(range(len(reward) - 1)):
        ...
        advantage[t][tt] = (
            reward[t][tt]
            + discount * value[t + 1][tt]  # Same formula before loop.
            - value[t][tt]
        )
    return_[:] = advantage + value
    return advantage, return_
