...
class SacWithUl(RlAlgorithm):
    ...
    def ul_optimize(self, itr, rl_samples=None):
        ...
        n_rl_samples = (
            0 if rl_samples is None else len(rl_samples.agent_inputs.observation)
        )
    ...
    def ul_optimize_one_step(self, samples=None):
        ...
        if samples is None:
            if self.ul_pri_alpha > 0:
                samples = self.replay_buffer.sample_batch(self.ul_batch_size, mode="UL")
            else:
