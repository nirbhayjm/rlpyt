...
class DDPG(RlAlgorithm):
    ...
    def optim_initialize(self, rank=0):
        ...
        self.rank = rank
        self.mu_optimizer = self.OptimCls(
            self.agent.mu_parameters(), lr=self.learning_rate, **self.optim_kwargs
        )
        self.q_optimizer = self.OptimCls(
            self.agent.q_parameters(), lr=self.q_learning_rate, **self.optim_kwargs
        )
    ...
    def optimize_agent(self, itr, samples=None, sampler_itr=None):
        ...
        for _ in range(self.updates_per_optimize):
            ...
            opt_info.qGradNorm.append(
                torch.tensor(q_grad_norm).item()
            ...
            if self.update_counter % self.policy_update_interval == 0:
                ...
                opt_info.muGradNorm.append(
                    torch.tensor(mu_grad_norm).item()
                )  # backwards compatible
